 ...
 * and open the template in the editor.
 */
$title="Sugestões Alunos";
include_once('Data\Conexao.php');
$pdo = Database::connect();
$sql="select * from recomendacoes as a, estudante as b where a.idAluno=b.idUsuario and statusRecomend=1 order by idRecomend desc";
...
$conteudo='<ol class="breadcrumb">
            <li><a href="homeSugestoes.php">Sugestões</a></li>
                ...
                <h3 style="text-align:center">Sugestões de Alunos</h3>
                <hr>';
foreach ($pdo->query($sql) as $row){
                $conteudo=$conteudo.'<dl class="dl-horizontal">
                    ...
                    <dd>
                        ...
                        $recomendacao=$row['idRecomend'];
                        $id=$_SESSION['id'];
                        $sql2=("select count(*) as qtd from levelup.favoritos where idAluno=".$id." and recomendAluno=".$recomendacao."");
                        $resultado = $pdo->query($sql2)->fetch();
                        $qtd=$resultado['qtd'];
                        
                        if($qtd>=1){
                            $conteudo=$conteudo.'<a href="#" class="btn btn-sm btn-danger" rel="tooltip" title="Remover dos Favoritos" data-toggle="modal" data-target=".bs-example-modal-sm2_'.$row['idRecomend'].'"> <i class="glyphicon glyphicon-heart"></i></a>
                        <div class="modal fade bs-example-modal-sm2_'.$row['idRecomend'].'" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel">
                                    ...
                                    <div style="padding: 8px 8px 8px 8px">
                                        ...
                                        <form action="removeFavorito.php" method="post">
                                            <div class="form-group">
                                                <input type="hidden" name="idRecAluno" value="'.$row['idRecomend'].'">
                                            </div>
                            ...
                            $conteudo=$conteudo.'<a href="#" class="btn btn-sm btn-danger" rel="tooltip" title="Adicionar aos Favoritos" data-toggle="modal" data-target=".bs-example-modal-sm1_'.$row['idRecomend'].'"><i class="glyphicon glyphicon-heart-empty"></i></a> 
